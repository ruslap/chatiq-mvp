...
export class ChatService {
  ...
  async saveVisitorMessage(
    ...
    if (!site) {
      this.logger.log(
        `Site ${siteId} not found, creating a default one for testing`,
      );
      ...
      if (user) {
        ...
        throw new InternalServerErrorException(
          'No users found in database to assign the new site to',
        );
      }
    }
    ...
    void this.automationService
      ...
      .catch((error) =>
        this.logger.error('Auto-reply execution failed', error.stack),
      );
